...
UENUM(BlueprintType)
enum class EEquipType :uint8  // 装备类型
{
	E_None UMETA(DisplayName = "无"),
	// 默认主手为右手
	E_MainHand UMETA(DisplayName = "主手"),
	// 副手为左手；特殊处理：当正常的弓装备时，仍然装备在副手
	E_OffHand UMETA(DisplayName = "副手"),
...
USTRUCT(BlueprintType)
struct FEquip : public FTableRowBase
{
	GENERATED_BODY()
public:
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
...
USTRUCT(BlueprintType)
struct FArmor : public FEquip
{
	GENERATED_BODY()

public:
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
		EEquipType ArmorType;
};
